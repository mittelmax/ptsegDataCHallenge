...
boruta = pd.read_csv(f"{data_dir}/borutaOutput/boruta_vars.csv")

# Filtering for selected vars
boruta_vars = boruta[boruta["Selected"] == 1]
boruta_vars = boruta_vars["feature"].values.tolist()
df = df[boruta_vars + ["y"]]


# Defining custom F1 metric for lightGBM
...
def objective(trial, seeds=[28, 194, 193, 405]):
    ...
    params = {
        ...
        "path_smooth": trial.suggest_uniform("learning_rate", 0, 0.1)}
...
filename = f"{data_dir}/borutaOutput/bestParamsBorutaLightGBM.pickle"
outfile = open(filename, "wb")
pickle.dump(best_params, outfile)
outfile.close()

best_iter = num_rounds[study.best_trial.number]
filename = f"{data_dir}/tuningBase/bestIterBorutaLightGBM.pickle"
outfile = open(filename, "wb")
